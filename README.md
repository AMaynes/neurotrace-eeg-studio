...
- Existing zoom/window controls automatically use the protected whole-recording index in unfiltered recorded-reference views when its resolution is sufficient (and always for the full recording). Enabling another channel requires no new whole-file read. EDF/DAT hashing and indexing share one source pass; MAT builds its index alongside verification. EDF, DAT, and MAT v5 publish completed exact prefixes during that first scan; unread regions are explicitly hatched, never drawn as zero signal. These indexes live in memory for open sessions, not persistent storage; reopening the file rebuilds them.
