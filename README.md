...
- Nearby data is reused from three bounded caches: 64 MiB raw windows, 64 MiB processed windows, and 256 MiB zoomed-out envelopes. Each new envelope pyramid is capped at 128 MiB to leave room for both a wide view and a close-up. Older entries are removed when a cache is full.
- Wide, unfiltered recorded-reference DAT/MAT v5 views reuse complete fixed-grid buckets during overlapping pans and read only newly exposed intervals when resolution and memory limits allow. Incremental windows keep one grid at roughly twice the display-column resolution, avoiding a synchronous pyramid rebuild or shifting bucket centers after each pan. Full-session indexes built during file verification retain their multiresolution levels. Reused extrema, gaps, units, and timing are unchanged. The first uncached interval still requires a source scan; filtered and derived montages keep their existing exact-sample processing and window limits.
